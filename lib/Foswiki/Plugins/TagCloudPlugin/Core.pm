...
sub handleTagCloud {
    ...
    if ( expandVariables($theTerms) ) {
        ...
        $theTerms =
          Foswiki::Func::expandCommonVariables( $theTerms, $theTopic, $theWeb );
    }
    ...
    foreach my $term ( keys %termCount ) {
        ...
        # normalization
        $termCount{$term} = log( $termCount{$term} )
          if $theNormalize == NORMALIZE_LOG;
    ...
    unless ( scalar( keys %termCount ) ) {
        return '<span class="foswikiAlert">nothing found</span>'
          if $theWarn eq 'on';
        return '';
    }
    ...
    foreach my $term ( keys %termCount ) {
        $termWeight{$term} =
          int( ( $termCount{$term} - $floor + 0.0 ) / $incr ) + 1;
    }
    ...
    if ( $theSort eq 'alpha' ) {
        ...
        if ( $theSort eq 'count' ) {
            @sortedTerms =
              sort { ( $termCount{$b} <=> $termCount{$a} ) || ( $a cmp $b ) }
              keys %termCount;
        }
        else {
            @sortedTerms =
              sort { ( $termWeight{$b} <=> $termWeight{$a} ) || ( $a cmp $b ) }
              keys %termCount;
        }
    }
    ...
    foreach my $term (@sortedTerms) {
        ...
        $text =~
s/\$fadeRGB\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/&handleFadeRGB($1,$2,$3,$4,$5,$6, $weight, $theBuckets+$theOffset)/ge;
        $result .= $text;
    }
...
sub handleFadeRGB {
    my ( $startRed, $startGreen, $startBlue, $endRed, $endGreen, $endBlue,
        $step, $max )
      = @_;

    my $red =
      int( $startRed * ( ( $max - $step + 0.0 ) / $max ) +
          $endRed * ( ( $step + 0.0 ) / $max ) );
    my $green =
      int( $startGreen * ( ( $max - $step + 0.0 ) / $max ) +
          $endGreen * ( ( $step + 0.0 ) / $max ) );
    my $blue =
      int( $startBlue * ( ( $max - $step + 0.0 ) / $max ) +
          $endBlue * ( ( $step + 0.0 ) / $max ) );
